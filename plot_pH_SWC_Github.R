...
path = '/Volumes/GoogleDrive/My Drive/KRACCEN (West Coast Kelp Forest Project)/Regions/Calibrated_pH_Data'
setwd(path)
...
for (i in 1:length(L$Dep)) {
  Dep = L$Dep[i]
  pHDat = read.csv(paste('/Volumes/GoogleDrive/My Drive/KRACCEN (West Coast Kelp Forest Project)/Regions/Calibrated_pH_Data/',L$Dep[i], sep=""),
                   skip = 4)
...
a <- ggplot(pHDatGood, aes(x = DT, y = pH_int, group = i)) + 
    geom_line() +
    theme_classic() +
    labs(x="Date", y = "pH", element_text(size = 18)) +
...
b <- ggplot(pHDatGood, aes(x = DT, y = Temperature, group = i)) + 
  geom_line() +
  theme_classic() +
  labs(x="Date",element_text(size = 18)) +
...
l
ggsave(plot = l, file = "SWC_timeseries_Month.png", 
...
p
ggsave(plot = p, file = "SWC_Reg_plots.png", 
       type = "cairo-png",  bg = "white",
...
summary(pHDO)
DOTC <- lm(pHDatGood_UpwellingAll$OxymgL ~ pHDatGood_UpwellingAll$Temperature)
summary(DOTC)
...
summary(pHDO)
DOTC <- lm(pHDatGood_NonUpwellingAll$OxymgL ~ pHDatGood_NonUpwellingAll$Temperature)
summary(DOTC)
...
d <- d + guides(colour = guide_colourbar(title.position="top",title.hjust =0.5))
d
...
e <- ggplot(pHDatGood_NonUpwellingAll, aes(x = pH_int, y = OxymgL, color = Temperature)) + 
  ...
  xlim(7.6, 8.1) +
  theme(plot.title = element_text(hjust = 0.5)) +
  #ggtitle("Non-Upwelling") +
  theme(legend.position = "right", legend.key.width = unit(1,'cm'))
e <- e + guides(colour = guide_colourbar(title.position="top",title.hjust =0.5))
e
